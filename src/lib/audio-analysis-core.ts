/**
 * Pure DSP core for track analysis: tempo (BPM), a beat-grid phase
 * reference, an "energy onset" heuristic for where a track's groove
 * actually starts (as opposed to a cold/silent intro), and a best-effort
 * musical key. Only touches a Float32Array of samples plus a sample rate —
 * no DOM, no Worker, no WASM-loading — so it's safe to import from
 * anywhere, including the analysis Web Worker (src/workers/analysis.worker.ts).
 *
 * Split out from audio-analysis.ts specifically so that file's Worker-
 * spawning code (`new Worker(new URL("../workers/analysis.worker.ts", ...))`)
 * and the worker's own `import { analyzeSamples } from ...` don't point at
 * the same file — that cycle (a worker entrypoint importing from the file
 * that spawns it) made Turbopack's production build hang indefinitely at
 * "Creating an optimized production build...", even though dev mode worked
 * fine. audio-analysis.ts re-exports everything here for backward
